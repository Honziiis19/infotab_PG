...
if(!( isset($_SESSION["logged_in"]) && $_SESSION["logged_in"] == 1)) { header("Location: login.php"); exit; };
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && $_POST["action"] == "submited") { // tohle už nechápu
        if (!$con) {
        ...
        if($row = mysqli_fetch_assoc($result)) { 
            ...
            header("location: account.php"); //aby se neuložilo dvakrat
            };
        }
    }
?>
<body>
    <h1 style= "font-family: monospace" class="text-center">Možnost změny hesla</h1>
